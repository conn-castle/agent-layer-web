...
Agent Layer is local-first: it writes config files in your repository and launches the client you choose. It avoids hidden defaults and fails with an explicit error instead of guessing.
...
Treat generated client outputs as build artifacts and keep them gitignored.

- `.codex/config.toml` may contain resolved MCP server URLs, commands, or environment variables. When `[agents.codex] local_config_dir = true`, `.codex/` is also used as `CODEX_HOME` and may contain Codex auth, sessions, logs, and other runtime state.
- When `[agents.claude] local_config_dir = true`, `.claude-config/` is used as `CLAUDE_CONFIG_DIR` and may contain user-level settings and caches.

Treat `.codex/` and `.claude-config/` as sensitive local state and keep them out of Git. The default template ignores both directories; preserve those rules if you customize the managed gitignore block.

Claude Code stores `/login` credentials in the encrypted macOS Keychain on macOS. On Linux and Windows, it stores them in `.credentials.json` under the default config directory, or under `CLAUDE_CONFIG_DIR` when that variable is set. Other authentication modes can use environment API keys or OAuth tokens, `apiKeyHelper`, or external credential sources for Bedrock, Vertex, and Foundry. See [Claude Code authentication](https://code.claude.com/docs/en/authentication).
