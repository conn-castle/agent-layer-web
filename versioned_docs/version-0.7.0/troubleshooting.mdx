...
`.agent-layer/al.version` must be `X.Y.Z` or `vX.Y.Z`. Fix the file or re-run `al init --version X.Y.Z`. See [Version pinning](./concepts#version-pinning) and [Upgrades](./upgrades).
