...
Agent Layer does not install Antigravity, Claude, Codex, Copilot CLI, or VS Code. Install the target client and ensure it is on your `PATH`.
...
Generated client outputs should stay gitignored. If you see files like `.agents/skills/`, `.agy/`, `.antigravitycli/`, `.claude/settings.json`, `.claude/skills/`, `.codex/`, `.copilot/`, `.vscode/mcp.json`, `.vscode/settings.json`, `.mcp.json`, `.github/copilot-instructions.md`, `AGENTS.md`, or `CLAUDE.md` showing up in `git status`, it usually means one of:
...
Install the target client CLI (Antigravity `agy`, Claude, Codex, Copilot CLI, or VS Code) and ensure it is available on your `PATH`.
