...
2. Parse instructions and load both canonical skill tiers into one locked source snapshot.
...
`.agent-layer/.gitignore` ignores both machine-managed paths by default. Agent
Layer never stages or commits your repository.
...
Each skill uses only its own lock entry as the upstream merge base. Members
declared together may therefore remain at different commits after partial
success. Newly matched skills are imported at that pull's current resolved
target without moving an existing pinned sibling.
...
The placeholder text is what stays canonical. It is what `config.toml` and
