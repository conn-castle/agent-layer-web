...
- `clients` to restrict which clients receive a server (valid values: `gemini`, `claude`, `vscode`, `codex`, `antigravity`, `copilot`; note that `claude_vscode` uses the `claude` client)
...
| `al <client>` | Sync and launch a client (gemini/claude/codex/copilot/vscode/antigravity). |
...
Skill sources align with the [agentskills.io specification](https://agentskills.io/specification), with explicit backward-compatibility behavior documented below.
...
- `.agents/skills/`
- `.gemini/settings.json`, `.claude/settings.json`, `.claude/skills/`, `.mcp.json`
