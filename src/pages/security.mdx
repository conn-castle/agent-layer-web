...
Treat generated client outputs as build artifacts and keep them gitignored. In particular, `.codex/config.toml` may contain resolved values for MCP server URLs, commands, or environment variables. When `local_config_dir = true` is enabled under `[agents.codex]`, `.codex/` is also used as `CODEX_HOME` and may contain Codex auth, sessions, logs, and other runtime state. When `local_config_dir = true` is enabled under `[agents.claude]`, the `.claude-config/` directory (set via `CLAUDE_CONFIG_DIR`) may contain user-level settings and caches. Treat `.codex/` and `.claude-config/` as sensitive local state and keep them out of Git (the default template ignores them; preserve those ignores if you customize managed gitignore rules). Note: Claude Code currently stores auth credentials in its OS credential store (macOS Keychain service `"Claude Code-credentials"`; Linux libsecret/gnome-keyring) regardless of `CLAUDE_CONFIG_DIR` (upstream limitation), so `.claude-config/` does not contain auth tokens.
