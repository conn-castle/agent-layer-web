...
The wizard's Rules and memory instruction choice creates `docs/agent-layer/` for repo-specific memory files. These files are long-lived, human-readable context that agents can reference. Keep them current so later work has context instead of guesswork.
...
When you choose Rules and memory in `al wizard`, it creates missing files:
