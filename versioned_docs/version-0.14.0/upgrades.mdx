...
- **Tmp deletion is gated.** Interactive deletion requires a destructive double-confirm. Non-interactive deletion requires the dedicated `--apply-tmp-deletions` flag — `--apply-deletions` alone never touches tmp content. The interactive "delete all unknowns?" prompt is similarly scoped: it deletes everything **except** tmp, and the grouped tmp prompt always fires separately.
...
| --- | --- | --- | --- | --- |
| `v0.14.0` | `0.10.2`, `0.11.0`, `0.12.0`, `0.12.1`, `0.12.2`, `0.12.3`, `0.13.0` | `breaking/manual`: Agent Dispatch now exposes asynchronous handle-based lifecycle commands: `start`, `wait`, `continue`, and `cancel`. The prior synchronous fresh/resume syntax and public `fanout`, `inspect`, `history`, `list`, and `delete` operations are removed. `safe auto`: retired per-agent dispatch default tables are removed before strict config decoding. | Update dispatch scripts and workflows to call `al dispatch start --agent <agent> --prompt <text>`, retain the returned `handle`, then call `al dispatch wait <handle>` for the terminal state and result path. Use `al dispatch continue <handle> --prompt <text>` only after the current invocation is terminal. Replace fanout with independent starts; do not poll for completion. | The migration manifest deletes `agents.antigravity.dispatch`, `agents.claude.dispatch`, and `agents.codex.dispatch`, including their `default_agent` values. It retains the established `0.10.2` migration floor so source-agnostic cleanup also runs for supported legacy upgrades. Every successful lifecycle command writes one JSON object; completed answers are immutable Markdown files named by `result_path`. |
