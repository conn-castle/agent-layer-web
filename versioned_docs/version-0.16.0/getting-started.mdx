...
This is enough to be productive immediately. The refinements (custom skills, additional MCP servers, version pinning) can be layered in as your needs grow.
...
- Install the target client (Antigravity `agy`, Claude, Codex, Copilot CLI, VS Code)
...
By default, `al init` prompts to run the setup wizard. The wizard can add the optional workflow bundle, including `docs/agent-layer/` project memory. Skip it with `--no-wizard` if you prefer to configure manually.
...
Any client command (agy/claude/codex/copilot/vscode) will:
...
`al init` creates an empty `[mcp]` section. Run `al wizard` to choose from the embedded MCP catalog, or add server blocks by hand. Two great first picks are:
...
Skill sources follow the [agentskills.io specification](https://agentskills.io/specification). User-managed skills use `.agent-layer/skills/<name>/SKILL.md`; imported skills use `.agent-layer/skills-imported/<name>/SKILL.md`. Both tiers require the uppercase canonical filename; lowercase `skill.md` is rejected.

### Project memory (`docs/agent-layer/`)

These files are created when you install the workflow bundle in `al wizard`. The bundled memory instructions reference them as long-lived memory for the repo:
...
- generated client outputs like `.agents/skills/`, `.agy/`, `.antigravitycli/`, `.claude/settings.json`, `.claude/skills/`, `.mcp.json`, `.codex/`, `.copilot/`, `.vscode/mcp.json`, `.vscode/settings.json`, `.github/copilot-instructions.md`, `AGENTS.md`, `CLAUDE.md`
