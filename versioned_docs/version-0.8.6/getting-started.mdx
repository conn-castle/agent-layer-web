...
:::note
The Codex VS Code extension reads `CODEX_HOME` at startup. Repo-local launchers set a per-repo value, so you will need to reauthenticate with Codex when opening a different repo. If you enable `local_config_dir = true` under `[agents.claude]` (and `[agents.claude-vscode]` is enabled), the Claude extension also receives a per-repo `CLAUDE_CONFIG_DIR`, providing isolated authentication and credential separation across repositories.
...
Enabling repo-local `CLAUDE_CONFIG_DIR` means Claude authentication state becomes repo-scoped, so expect a one-time reauthentication the first time you open each repository after setting `local_config_dir = true`.
:::
