...
- Codex agent-specific values are written at the TOML root before managed project trust and MCP tables so top-level overrides stay explicit.
...
**Codex** isolation is automatic. `al codex` and repos that enable `[agents.vscode]` use a repo-local `CODEX_HOME`. The generated `.codex/config.toml` also includes a trust stanza for the current absolute repo root, so Codex authentication and trust are per-repo with no extra configuration.
...
| `yolo` | auto-approve | auto-approve | skips all permission prompts where supported (Claude `--dangerously-skip-permissions`, Codex `approval_policy=never` + `sandbox_mode=danger-full-access` + `web_search=live`, Copilot CLI `--yolo`, VS Code `chat.tools.global.autoApprove`) |
...
When selected in `al wizard`, the workflow bundle creates:
