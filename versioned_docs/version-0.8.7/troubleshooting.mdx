...
When `local_config_dir` is enabled, `al claude` sets a repo-local `CLAUDE_CONFIG_DIR`, so Claude Code uses the repo-local `.claude-config/` for user-level configuration (authentication, caches, settings). For `al vscode`, `CLAUDE_CONFIG_DIR` is set only when both `local_config_dir` is `true` and `[agents.claude-vscode]` is enabled.

If you launch VS Code without Agent Layer's repo-local launcher, the Claude extension will not receive the repo-local `CLAUDE_CONFIG_DIR`. In that case, the extension uses your global `~/.claude/` configuration, so authentication and settings are shared across repositories.
...
- Claude auth/state is shared across repositories (which can be convenient, but is less isolated).
- You cannot use different Claude accounts per repo.
