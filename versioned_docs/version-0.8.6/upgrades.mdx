...
| --- | --- | --- | --- | --- |
| `v0.8.6` | `0.8.x`, `0.7.x` (via migration chaining) | `safe auto`: Fixed `.gitignore` template where inline comments on `/.claude/` and `/.claude-config/` patterns caused Git to treat them as literal pattern text, leaving both directories unignored. `al wizard` now prompts for per-repo Claude credential isolation (`local_config_dir`). Documentation now highlights per-repo credential isolation as a core feature. | No manual action required; run `al upgrade plan` then `al upgrade`. Users who installed v0.8.5 in a new repo should verify `.claude/` is gitignored after upgrading. | No config schema changes. Template hash will change due to the gitignore fix. |
