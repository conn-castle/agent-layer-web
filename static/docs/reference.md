...
DeltaSelect exports a content-addressed benchmark selection and a matching `study.toml` template. Keep both files together, define explicit baseline and treatment experiments in the template, and validate the complete study before authorizing provider calls:

```bash
al benchmark run study.toml --dry-run
al benchmark run study.toml
```
...
Use `--task-concurrency 1..8` to control parallel cells. The repeatable `--task <id>` flag scopes one invocation to selected tasks without changing study membership or identity. Run `al benchmark run --help` for the exact flags and use [DeltaSelect](/deltaselect) to create the selection and study template.
