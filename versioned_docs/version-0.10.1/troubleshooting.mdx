...
Agent Layer does not install Gemini, Claude, Codex, Copilot CLI, VS Code, or Antigravity. Install the target client and ensure it is on your `PATH`.
...
Generated client outputs are meant to be disposable. If you see generated files like `.agents/skills/`, `.gemini/settings.json`, `.claude/settings.json`, `.claude/skills/`, `.codex/`, `.copilot/`, `.vscode/mcp.json`, `.vscode/settings.json`, `.mcp.json`, `.github/copilot-instructions.md`, `AGENTS.md`, `CLAUDE.md`, or `GEMINI.md` showing up in `git status`, it usually means one of:
...
Install the target client CLI (Gemini, Claude, Codex, Copilot CLI, VS Code, Antigravity) and ensure it is available on your `PATH`.
