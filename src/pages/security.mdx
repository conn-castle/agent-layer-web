...
Agent Layer is local-first: it generates deterministic config files into your repository and launches the client you choose. It avoids hidden defaults and prefers failing loudly over "best guess" behavior.
...
- **Pinned version downloads** (GitHub Releases). If a repo pins `.agent-layer/al.version`, `al` may download that version into your local cache and dispatch to it.
...
Treat generated client outputs as build artifacts and keep them gitignored. In particular, `.codex/config.toml` may contain resolved values for MCP server URLs, commands, or environment variables.
