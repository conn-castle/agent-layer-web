...
Run [`al init`](./reference#init) in the repo to create `.agent-layer/`. To add `docs/agent-layer/` memory files, run [`al wizard`](./reference#wizard) and select the workflow bundle.
...
Continue using `al codex` on the CLI. `al codex` sets a repo-local `CODEX_HOME`, so Codex uses the repo-local `.codex/config.toml` (which Agent Layer generates from `.agent-layer/config.toml`, including the current repo trust stanza and MCP servers).
