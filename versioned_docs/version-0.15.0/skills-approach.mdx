...
for plan_reviewer in plan_reviewers:
    dispatch(plan_reviewer) -> /review-plan
```

Each named reviewer runs `/review-plan` directly through one to three
risk-based framings of the complete artifact set. Separate dispatched reviewers
provide independent contexts; multiple framings within one reviewer broaden
coverage but are not independent reviews. The planner independently judges the
resulting findings; reviewer agreement is not evidence.
...
| --- | --- |
| `review-plan` | Make one risk-proportional pass over plan artifacts and report only evidence-backed, material findings. |
