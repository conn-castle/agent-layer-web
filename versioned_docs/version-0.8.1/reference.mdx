...
- `clients` to restrict which clients receive a server (valid values: `gemini`, `claude`, `vscode`, `codex`, `antigravity`; note that `claude-vscode` uses the `claude` client)
...
- Supports manual restore of applied snapshots via `al upgrade rollback <snapshot-id>`
...
- Readiness checks (for example unresolved placeholders, process-env vs `.env` collisions, ignored empty `.env` assignments, path-expansion anomalies, stale VS Code `--no-sync` outputs, floating dependency specs, stale disabled-agent artifacts, and generated-secret risk warnings)
...
- `auto-approve` (optional, defaults to `false`): when `true`, auto-approves the MCP prompt retrieval for that skill in Claude clients. It does **not** auto-approve agent actions after reading the prompt; those remain governed by `approvals.mode`
