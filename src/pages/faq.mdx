...
Yes. Skill parsing and validation align with the [agentskills.io specification](https://agentskills.io/specification), including canonical directory `SKILL.md` naming with a compatibility fallback for lowercase `skill.md`.

## Does Agent Layer install Claude/Codex/Gemini/Copilot CLI/VS Code?
...
- Agent config: `.mcp.json`, `.gemini/settings.json`, `.claude/settings.json`, `.copilot/mcp-config.json`
...
See [/docs/upgrades#ephemeral-artifacts-and-rollback-scope](/docs/upgrades#ephemeral-artifacts-and-rollback-scope) for the full contract.
