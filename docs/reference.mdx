...
# 3 allows two nested `al dispatch` calls.
max_depth = 3
...
[agents.antigravity]
enabled = true
# Optional default when Antigravity calls `al dispatch` without --agent.
...
# Optional default when Claude calls `al dispatch` without --agent.
...
# Optional default when Codex calls `al dispatch` without --agent.
...
| `al dispatch` | Run a supported headless target (`codex`, `claude`, `antigravity`, or `random`) from an Agent Layer-launched caller. |
| `al dispatch fanout` | Send one shared prompt and optional skill to two or more self-contained target specifications, wait for all children, and emit one manifest. |
| `al dispatch inspect <name-or-id>` | Report read-only factual state and diagnostics for one run or conversation. |
| `al dispatch history <name>` | Show retained immutable turns for one friendly conversation name. |
| `al dispatch cancel <id>` | Cancel an exact active run, conversation, or fanout process group. |
| `al dispatch options --json` | Print the stable Agent Dispatch target/options contract for scripts. |
...
`al dispatch` starts one fresh, final-answer-only headless provider turn:
...
Supported target values are `codex`, `claude`, `antigravity`, and `random`.
When `--agent` is omitted, Agent Layer uses the env-only caller marker
(`AL_DISPATCH_CALLER_AGENT`) and the caller's optional
`agents.<caller>.dispatch.default_agent`, defaulting to `random`.

Each ordinary call creates a new provider conversation. Only `resume <name>`
continues a conversation, using an exact durable mapping; prompt, role, target,
and artifact paths never infer reuse. Dispatch writes one identity line to
standard error and replays only a semantically completed final answer to
standard output. `fanout` is a synchronous shared-prompt operation that stores
independent child results and emits one aggregate manifest after every child is
terminal. `inspect` reports read-only transport facts, `history` derives turns
from immutable records, `cancel` targets an exact owned process group, and `delete`
removes only the Agent Layer mapping. Use `al dispatch options --json` for
installed version and separate fresh/resume/inspect capability facts. The
tested compatibility set is Claude Code 2.1.207, Codex CLI 0.144.1, and
Antigravity 1.1.1; newer installed versions dispatch with a compatibility
warning, while older or unreadable versions fail before launch. See
`docs/AGENT-DISPATCH.md` in the repository for the full v0.13 contract.

**Exit codes**

- `0` — dispatch completed successfully
- `64` — usage or resolution failure (missing prompt and skill, unknown target, unknown caller with omitted `--agent`, unknown dispatch/session name for resume, inspect, or delete, unsupported override field for the selected target)
- `65` — configuration or state failure (missing source skill, stale/missing target skill projection, malformed config, disabled target)
- `69` — target unavailable (missing target binary, unsupported provider version, no eligible target in the random-selection pool, attempt to delete an active session)
- `70` — target or adapter failure (provider failure, missing required terminal result, capture limit/I/O failure, or process-supervision failure)
