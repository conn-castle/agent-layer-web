---
title: FAQ
description: Answers about Agent Dispatch, portable Agent Skills, MCP configuration, supported coding agents, security, installation, and upgrades.
keywords:
  - Agent Layer FAQ
  - Agent Dispatch
  - import Agent Skills
...
## Can one coding agent delegate work to another provider?

Yes. [Agent Dispatch](/docs/agent-dispatch) lets supported callers with working MCP support start asynchronous Claude, Codex, Grok, or Antigravity conversations through built-in MCP tools. People and automation can use matching `al dispatch` CLI commands. The caller receives a handle immediately, waits when it needs the result, and can continue the same provider conversation later.

Antigravity remains a supported dispatch target, but its current probe baseline does not expose the generated MCP tools at runtime. Run `al probe agy` before relying on Antigravity as an MCP caller.

## Can I import skills from another Git repository?

Yes. [`al skills add`](/docs/skill-imports) imports selected Agent Skills from any reachable Git repository. Imported skills remain editable in your project, project to every enabled client, and use three-way reconciliation so compatible upstream and local changes both survive. You can also configure an explicit branch or fork for contributing improvements back.

## Does Agent Layer align with the skills standard?
Yes. Skill parsing and validation follow the [agentskills.io specification](https://agentskills.io/specification), including the required uppercase `SKILL.md` filename.
...
By default (`agents.codex.local_config_dir` absent or `false`), Agent Layer does not set `CODEX_HOME`; Codex uses its normal global/project config layering and any inherited `CODEX_HOME` remains user-owned. If you prefer global Codex MCP config in VS Code, see [/docs/troubleshooting#global-mcp-server-fallback-for-vs-code-codex](/docs/troubleshooting#global-mcp-server-fallback-for-vs-code-codex).

## Does Claude keep separate sign-ins per repo?
Claude per-repo settings, caches, and `/login` credential isolation is opt-in. If you set `local_config_dir = true` under `[agents.claude]` in `config.toml`, Agent Layer sets `CLAUDE_CONFIG_DIR` to a repo-local directory, isolating settings, caches, and `/login` credentials per repository.

Authentication isolation depends on your platform and authentication method. On macOS, Claude Code keys its Keychain entry to `CLAUDE_CONFIG_DIR` and falls back to `.credentials.json` there if the Keychain write fails. On Linux and Windows, it stores them in `.credentials.json` under `CLAUDE_CONFIG_DIR`, so a repo-local directory also isolates those credentials. An explicit `CLAUDE_SECURESTORAGE_CONFIG_DIR` can override the credential directory. Environment credentials, `apiKeyHelper`, and cloud-provider authentication can come from external sources. See [Claude Code authentication](https://code.claude.com/docs/en/authentication).
...
For compatibility guarantees and release migration rules, see [/docs/upgrades](/docs/upgrades). For a one-page runbook, use [/docs/upgrade-checklist](/docs/upgrade-checklist).
...
See [/docs/upgrades](/docs/upgrades) for rollback scope and the rest of the upgrade policy.
